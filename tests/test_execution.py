...
class TestKernelExecution:
    ...
    def test_create_compute_pipeline_state(self, device):
        ...
        assert function is not None, "Function 'vector_add' should be found in the library"
        
        pipeline_state, error = device.newComputePipelineStateWithFunction_error_(function, None)
        
        assert pipeline_state is not None, "MTLComputePipelineState should not be None"
        assert error is None, f"Error should be None, but got: {error.localizedDescription() if error else 'Unknown Error'}"
        
        assert pipeline_state.threadExecutionWidth() > 0, "Thread execution width should be positive"
        assert pipeline_state.maxTotalThreadsPerThreadgroup() > 0, "Max threads per threadgroup should be positive"
    ...
    def test_create_pipeline_state_with_nonexistent_function(self, device):
        ...
        assert nonexistent_function is None, "Should return None for a function that does not exist"
