...
def test_graph_structure():
    ...
    comp_nodes = [n for n in builder.graph.topo_sort() if n.op != "input"]
    add_node = comp_nodes[0]
    mul_node = comp_nodes[1]

    assert add_node.op == "add"
    assert mul_node.op == "mul"
    assert mul_node.inputs[0] is add_node
