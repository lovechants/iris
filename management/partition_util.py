...
class GPUPartitioner:
    ...
    def _query_gpu_properties(self) -> Dict[str, Any]:
        ...
        try:
            ...
            properties["max_total_threads_per_threadgroup"] = max_total_threads_size.width
        except AttributeError:
            properties["max_total_threads_per_threadgroup"] = properties["max_threads_per_threadgroup"]
        ...
        try:
            properties["recommended_max_working_set_size"] = device.recommendedMaxWorkingSetSize()
        except AttributeError:
        ...
        properties["max_threadgroups_per_grid"] = (
    ...
    def recommend_block_size(self, 
                       threads_needed: int, 
                       memory_per_thread: int = 0,
                       prefer_power_of_two: bool = True) -> int:
        """
        ...
        if memory_per_thread > 0:
            max_memory_threads = self.gpu_properties["threadgroup_memory_size"] // memory_per_thread
        ...
        if prefer_power_of_two:
            ...
            if recommended_block_size > thread_execution_width:
                recommended_block_size = (recommended_block_size // thread_execution_width) * thread_execution_width
            elif recommended_block_size < thread_execution_width and threads_needed >= thread_execution_width:
    ...
    def recommend_grid_size(self, 
                          total_elements: int, 
                          block_size: int,
                          max_grid_dimension: Optional[int] = None) -> int:
        """
    ...
    def partition_1d(self, 
                    data_size: int, 
                    memory_per_thread: int = 0,
                    max_grid_size: Optional[int] = None) -> Tuple[Tuple[int], Tuple[int]]:
        """
    ...
    def partition_2d(self, 
                    width: int, 
                    height: int, 
                    memory_per_thread: int = 0) -> Tuple[Tuple[int, int], Tuple[int, int]]:
        """
    ...
    def explain_partition(self, 
                         data_size: int,
                         grid_size: Tuple[int, ...],
                         block_size: Tuple[int, ...]) -> str:
        """
    ...
    def print_device_info(self):
        ...
        print(f"Max Total Threads per Threadgroup: {props['max_total_threads_per_threadgroup']}")
        print(f"Threadgroup Memory Size: {props['threadgroup_memory_size']} bytes")
        print(f"Recommended Max Working Set Size: {props['recommended_max_working_set_size']} bytes")
...
def partition_1d(data_size: int, memory_per_thread: int = 0, max_grid_size: Optional[int] = None) -> Tuple[Tuple[int], Tuple[int]]:
    """
    Partition a 1D workload into optimal grid and block sizes.
...
def partition_2d(width: int, height: int, memory_per_thread: int = 0) -> Tuple[Tuple[int, int], Tuple[int, int]]:
    """
    Partition a 2D workload into optimal grid and block sizes.
...
def explain_partition(data_size: int, grid_size: Tuple[int, ...], block_size: Tuple[int, ...]) -> str:
    """
    Generate a human-readable explanation of the partitioning.
