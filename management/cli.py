...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Metal Runtime for GPU Computing", prog="iris"
    )

    parser.add_argument("--info", action="store_true", help="Display GPU information")
...
def handle_recommendation():
    ...
    try:
        ...
        print(
            f"  - Use block sizes that are multiples of {thread_width} for optimal performance"
        )
        print(
            f"  - Examples: {thread_width}, {thread_width * 2}, {thread_width * 4}, etc."
        )
        print()
        ...
        print(
            f"  - Each thread can use up to ~{memory_size // max_threads} bytes if fully utilizing the threadgroup"
        )
        print()
        ...
        if "max_buffer_length" in props:
            ...
            print(
                f"• Max Buffer Length: {max_buffer:,} bytes ({max_buffer // (1024 * 1024):.1f} MB)"
            )
